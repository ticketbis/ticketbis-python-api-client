...
class Ticketbis(object):
    ...
    def __init__(
        ...
        self.oauth = self.OAuth(
            api_endpoint, client_id, client_secret, redirect_uri, grant_type
        )
        # Set up endpoints
    ...
    def rate_remaining(self):
        """Return the remaining rate limit for the last API call.
    ...
    class OAuth(object):
        """Handle OAuth authentication procedures and helps retrieve tokens"""

        def __init__(
            self, api_endpoint, client_id, client_secret, redirect_uri, grant_type
        ):
        ...
        def auth_url(self):
            """Get the url a user needs to access to give up a user token"""
            params = {
        ...
        def get_token(self, code=None, scope="read write"):
            """Get the auth token from a user's response"""
            params = {
                "client_id": self.client_id,
    ...
    class Requester(object):
        ...
        def _get_url(self, path):
            return "{API_ENDPOINT}{path}".format(
                API_ENDPOINT=self.api_endpoint, path=path
            )
    ...
    class _Endpoint(object):
        ...
        def __init__(self, requester):
            """Store the request function for retrieving data"""
        ...
        def GET(self, path=None, auto_pagination=False, *args, **kwargs):
            ...
            if not auto_pagination:
                ...
                return self.requester.GET_PAGINATED(
                    self._expanded_path(path), *args, **kwargs
                )
    ...
    class Events(_Endpoint):
        ...
        def __call__(self, event_id=u"", auto_pagination=False, params={}, multi=False):
            return self.GET(
                "{0}".format(event_id), auto_pagination, params=params, multi=multi
            )

        def section_groups(
            self, event_id, auto_pagination=False, params={}, multi=False
        ):
            return self.GET(
                "{0}/section_groups".format(event_id),
                auto_pagination,
                params=params,
                multi=multi,
            )
    ...
    class Categories(_Endpoint):
        ...
        def __call__(
            self, category_id=u"", auto_pagination=False, params={}, multi=False
        ):
            return self.GET(
                "{0}".format(category_id), auto_pagination, params=params, multi=multi
            )

        def events(self, category_id, auto_pagination=False, params={}, multi=False):
            return self.GET(
                "{0}/events".format(category_id),
                auto_pagination,
                params=params,
                multi=multi,
            )
    ...
    class Sites(_Endpoint):
        ...
        def __call__(self, site_id=u"", auto_pagination=False, params={}, multi=False):
            return self.GET(
                "{0}".format(site_id), auto_pagination, params=params, multi=multi
            )

    class Cities(_Endpoint):
        endpoint = "cities"

        def __call__(self, city_id=u"", auto_pagination=False, params={}, multi=False):
            return self.GET(
                "{0}".format(city_id), auto_pagination, params=params, multi=multi
            )

    class Venues(_Endpoint):
        endpoint = "venues"

        def __call__(self, venue_id=u"", auto_pagination=False, params={}, multi=False):
            return self.GET(
                "{0}".format(venue_id), auto_pagination, params=params, multi=multi
            )

        def schemas(self, venue_id, auto_pagination=False, params={}, multi=False):
            return self.GET(
                "{0}/schemas".format(venue_id),
                auto_pagination,
                params=params,
                multi=multi,
            )
    ...
    class Schemas(_Endpoint):
        ...
        def __call__(
            self, schema_id=u"", auto_pagination=False, params={}, multi=False
        ):
            return self.GET(
                "{0}".format(schema_id), auto_pagination, params=params, multi=multi
            )
    ...
    try:
        ...
        if response.status_code == 412:
            ...
            _log_and_raise_exception(
                "Precondition failed", response.text, cls=PreconditionFailed
            )
        ...
        for k, v in query:
            ...
            if isinstance(v, six.string_types):
                ...
                try:
                    ...
                    for elt in v:
                        args.append(k + "=" + parse.quote(_as_utf8(elt)))
    return "&".join(args)
